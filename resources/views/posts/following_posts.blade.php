...
@foreach($follower as $follow)
	@foreach($follow->post_follower as $post)
             ...
             <img src="{{url($post->user->image)}}" class='rounded-circle user-image'  style="max-height: 60px">
              ...
              -->
                @if($post->user->user_follower->where('user_id', auth()->user()->id)->first())
                 <a href="{{route('follow')}}" class='btn-follow' post_user="{{$post->user_id}}" style="position: absolute; right: 70px; top: 15px "><i class="fa fa-check"></i></a>
                @else
              		  <a href="{{route('follow')}}" class='btn-follow'
